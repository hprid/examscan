...
class SVG:
    ...
    def render(self):
        attrs = ' '.join('%s="%s"' % attr for attr in self.attrs.items())
        children = '\n'.join(c.render() for c in self.children)
        return '<%s %s>%s</%s>' % (self.tag_name, attrs, children, self.tag_name)
...
class PatternMaker:
    ...
    def save(self):
        c = SVG('svg', width="%d%s" % (self.width, self.units), height="%d%s" % (self.height, self.units),
